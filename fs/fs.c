...
static const char *k_module_name = "fs";
static const char *k_module_description = "Bolun Phone 1.0 bolun file store module";
...
int bolun_fs_route(BolunFSState *state, const char *command) {
    if (bolun_fs_health(state) < 0 || command == NULL || command[0] == '\0') {
        return -1;
    }
    state->generation += 1;
