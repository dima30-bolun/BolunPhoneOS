...
static const char *k_module_name = "camera";
static const char *k_module_description = "Bolun Phone 1.0 camera service module";
...
int bolun_camera_route(BolunCAMERAState *state, const char *command) {
    if (bolun_camera_health(state) < 0 || command == NULL || command[0] == '\0') {
        return -1;
    }
    state->generation += 1;
