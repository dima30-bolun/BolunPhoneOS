...
static const char *k_module_name = "sdk";
static const char *k_module_description = "Bolun Phone 1.0 developer sdk module";
...
int bolun_sdk_route(BolunSDKState *state, const char *command) {
    if (bolun_sdk_health(state) < 0 || command == NULL || command[0] == '\0') {
        return -1;
    }
    state->generation += 1;
