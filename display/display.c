...
static const char *k_module_name = "display";
static const char *k_module_description = "Bolun Phone 1.0 display server module";
...
int bolun_display_route(BolunDISPLAYState *state, const char *command) {
    if (bolun_display_health(state) < 0 || command == NULL || command[0] == '\0') {
        return -1;
    }
    state->generation += 1;
