...
static const char *k_module_name = "mm";
static const char *k_module_description = "Bolun Phone 1.0 memory manager module";
...
int bolun_mm_route(BolunMMState *state, const char *command) {
    if (bolun_mm_health(state) < 0 || command == NULL || command[0] == '\0') {
        return -1;
    }
    state->generation += 1;
