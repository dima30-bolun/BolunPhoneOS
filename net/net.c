...
static const char *k_module_name = "net";
static const char *k_module_description = "Bolun Phone 1.0 network stack module";
...
int bolun_net_route(BolunNETState *state, const char *command) {
    if (bolun_net_health(state) < 0 || command == NULL || command[0] == '\0') {
        return -1;
    }
    state->generation += 1;
