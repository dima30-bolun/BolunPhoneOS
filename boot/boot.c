...
static const char *k_module_name = "boot";
static const char *k_module_description = "Bolun Phone 1.0 boot chain module";
...
int bolun_boot_route(BolunBOOTState *state, const char *command) {
    if (bolun_boot_health(state) < 0 || command == NULL || command[0] == '\0') {
        return -1;
    }
    state->generation += 1;
