...
static const char *k_module_name = "hal";
static const char *k_module_description = "Bolun Phone 1.0 hardware abstraction module";
...
int bolun_hal_route(BolunHALState *state, const char *command) {
    if (bolun_hal_health(state) < 0 || command == NULL || command[0] == '\0') {
        return -1;
    }
    state->generation += 1;
