...
static const char *k_module_name = "shell";
static const char *k_module_description = "Bolun Phone 1.0 bolun shell module";
...
int bolun_shell_route(BolunSHELLState *state, const char *command) {
    if (bolun_shell_health(state) < 0 || command == NULL || command[0] == '\0') {
        return -1;
    }
    state->generation += 1;
