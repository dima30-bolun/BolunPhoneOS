...
static BolunResult add_subsystem(BolunKernel *kernel, const BolunSubsystemDescriptor *descriptor) {
    if (kernel->subsystem_count >= BOLUN_MAX_SUBSYSTEMS) {
        return BOLUN_ERROR_FULL;
    }
    kernel->subsystems[kernel->subsystem_count++] = *descriptor;
    return BOLUN_OK;
}

static BolunResult install_default_subsystems(BolunKernel *kernel) {
    const BolunSubsystemDescriptor defaults[] = {
        bolun_ai_subsystem(),
        bolun_apps_subsystem(),
        bolun_battery_subsystem(),
        bolun_bid_subsystem(),
        bolun_bluetooth_subsystem(),
        bolun_cloud_subsystem(),
        bolun_crypto_subsystem(),
        bolun_input_subsystem(),
        bolun_installer_subsystem(),
        bolun_notifications_subsystem(),
        bolun_permissions_subsystem(),
        bolun_phone_subsystem(),
        bolun_services_subsystem(),
        bolun_shell_subsystem(),
        bolun_themes_subsystem(),
        bolun_usb_subsystem(),
        bolun_vpn_subsystem(),
        bolun_wifi_subsystem(),
        bolun_youtube_subsystem(),
        bolun_audio_subsystem(),
        bolun_bapp_subsystem(),
        bolun_camera_subsystem(),
        bolun_drivers_subsystem(),
        bolun_filesystem_subsystem(),
        bolun_graphics_subsystem(),
        bolun_network_subsystem(),
        bolun_power_subsystem(),
        bolun_security_subsystem(),
        bolun_store_subsystem(),
        bolun_update_subsystem(),
    };

    for (size_t index = 0; index < sizeof(defaults) / sizeof(defaults[0]); ++index) {
        if (!kernel->device_profile.developer_mode_enabled && defaults[index].state == BOLUN_SUBSYSTEM_DEVELOPER_ONLY) {
            continue;
        }
        BolunResult result = add_subsystem(kernel, &defaults[index]);
        if (result != BOLUN_OK) {
            return result;
        }
    }
...
BolunResult bolun_kernel_boot(BolunKernel *kernel) {
    ...
    if (!kernel->booted) {
        BolunResult result = install_default_subsystems(kernel);
        if (result != BOLUN_OK) {
            return result;
        }
        result = install_default_features(kernel);
        if (result != BOLUN_OK) {
            return result;
        }
...
BolunResult bolun_kernel_get_status(const BolunKernel *kernel, BolunSystemStatus *out_status) {
    ...
    out_status->subsystem_count = kernel->subsystem_count;
...
BolunResult bolun_kernel_list_subsystems(const BolunKernel *kernel, BolunSubsystemDescriptor *out_subsystems, size_t capacity, size_t *out_count) {
    ...
    *out_count = kernel->subsystem_count;
    if (out_subsystems == NULL) {
        return BOLUN_OK;
    }
    size_t count = kernel->subsystem_count < capacity ? kernel->subsystem_count : capacity;
    memcpy(out_subsystems, kernel->subsystems, count * sizeof(BolunSubsystemDescriptor));
    return BOLUN_OK;
}
