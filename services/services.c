...
static const char *k_module_name = "services";
static const char *k_module_description = "Bolun Phone 1.0 system services module";
...
int bolun_services_route(BolunSERVICESState *state, const char *command) {
    if (bolun_services_health(state) < 0 || command == NULL || command[0] == '\0') {
        return -1;
    }
    state->generation += 1;
