...
static const char *k_module_name = "security";
static const char *k_module_description = "Bolun Phone 1.0 security core module";
...
int bolun_security_route(BolunSECURITYState *state, const char *command) {
    if (bolun_security_health(state) < 0 || command == NULL || command[0] == '\0') {
        return -1;
    }
    state->generation += 1;
