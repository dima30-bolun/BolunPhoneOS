...
static const char *k_module_name = "audio";
static const char *k_module_description = "Bolun Phone 1.0 audio server module";
...
int bolun_audio_route(BolunAUDIOState *state, const char *command) {
    if (bolun_audio_health(state) < 0 || command == NULL || command[0] == '\0') {
        return -1;
    }
    state->generation += 1;
