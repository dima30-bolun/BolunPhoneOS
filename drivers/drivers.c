...
static const char *k_module_name = "drivers";
static const char *k_module_description = "Bolun Phone 1.0 driver layer module";
...
int bolun_drivers_route(BolunDRIVERSState *state, const char *command) {
    if (bolun_drivers_health(state) < 0 || command == NULL || command[0] == '\0') {
        return -1;
    }
    state->generation += 1;
